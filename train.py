...
def meta_train(tasks, method='random', custom_task_ratio=None, meta_iters=1000, num_updates=1, num_samples=4, meta_batch_size=1):
    ...
    for i in range(meta_iters):
        ...
        for task_sample in range(meta_batch_size):
            ...
            # TODO change task sampler get task method
            ...
            for update in range(num_updates):
                ...
                predictions = task_model(sentences, task.get_name(), attention_mask=attention_masks)

                task_loss = task.get_loss(predictions, labels.to(device))
                task_loss.backward()
                task_optimizer.step()
